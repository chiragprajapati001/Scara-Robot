...
int max_srv_angle = 180, min_srv_angle = 0, srv_home = 160;
...
void loop() {
  if (isRecording) {
    ...
    if (digitalRead(buttonServoCW) == LOW) { // If CW button is pressed
      ...
      if (srv_pos > min_srv_angle) {                    // Ensure servo doesn't go below 0 degrees
        srv_pos -= 5;
        Servo1.write(srv_pos);
        delay(srv_del); // Small delay for smooth movement
      }
...
void goHome() {
    replay_servo(srv_home); //Servo Homed
...
void replay_servo(int go_pos)
{
  ...
  if (abs(current_pos - go_pos) > movementThreshold) {
